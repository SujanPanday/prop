...
function getGrade(total){if(total>=90)return{grade:'A+',label:'Elite Buy',cls:'g-aplus',chipSuffix:'aplus'};if(total>=75)return{grade:'A',label:'Strong Buy',cls:'g-a',chipSuffix:'a'};if(total>=60)return{grade:'B',label:'Good Opportunity',cls:'g-b',chipSuffix:'b'};if(total>=45)return{grade:'C',label:'Watchlist',cls:'g-c',chipSuffix:'c'};return{grade:'D',label:'Reject',cls:'g-d',chipSuffix:'d'};}
...
function totalScoreColor(total){if(total>=90)return'var(--grade-aplus)';if(total>=75)return'var(--grade-a)';if(total>=60)return'var(--grade-b)';if(total>=45)return'var(--grade-c)';return'var(--grade-d)';}